      ...
      <div className="container mx-auto px-0 h-full flex justify-between items-center">
        ...
        <div className="flex-1 flex justify-start">
          <Link href="/">
            <Image
              src="/images/logo.png"
              alt="Logo de Cafu Conexiones Sanitarias"
              width={140}
              height={40}
              priority
            />
        ...
        {/* --- Grupo Centro (Menú de Navegación) --- */}
          ...
          <nav className="flex items-center gap-8 h-full">
            {navLinks.map(link => (
              <Link
                key={link.href}
                href={link.href}
                className="text-base-text hover:text-primary transition-colors whitespace-nowrap"
              >
                {link.label}
              </Link>
            ))}
            ...
            <div className="relative group h-full flex items-center">
              <Link
                href="/productos"
                className="text-base-text hover:text-primary transition-colors"
              >
                Productos
              </Link>
              <div className="absolute top-full left-1/2 -translate-x-1/2 w-56 bg-white shadow-lg rounded-b-lg p-4
        ...
        <div className="hidden md:flex flex-1 items-center justify-end gap-1">
          ...
          >
            <Search size={20} />
            <span className="text-base italic whitespace-nowrap">Buscar productos...</span>
          </Link>

          <Link
            href="https://sistema.cliente.com/login"
            target="_blank"
            className="bg-secondary text-white font-bold py-3 px-6 rounded-full hover:bg-opacity-90 transition-all transform hover:scale-105 whitespace-nowrap"
          >
            Área de Clientes
          </Link>
        </div>
